...
function parseSplitMembers(raw: string | null | undefined): string[] {
  ...
  try {
    ...
    const members = parsed.members
      .map((entry) => sanitizeLabel(entry, '', 60))
      .filter(Boolean)
...
serve(async (req: Request) => {
  ...
  if (incomingCart.length > MAX_CART_ITEMS) {
    return json({ error: `Cart exceeds maximum of ${MAX_CART_ITEMS} items.` }, 400)
  }
  ...
  if (partySessionId) {
    ...
    if (sessionError || !sessionRow) {
      return json({ error: 'Party session not found.' }, 404)
    }

    if (sessionRow.status !== 'open') {
      return json({ error: 'This party session is not accepting payments.' }, 400)
    }

    restaurantId = Number(sessionRow.restaurant_id)
    if (!Number.isFinite(restaurantId) || restaurantId <= 0) {
      return json({ error: 'Invalid party session restaurant.' }, 400)
    }
    ...
    if (rows.length === 0) {
      return json({ error: 'No billable items were found for this party.' }, 400)
    }
    ...
    for (const row of rows) {
      ...
      const payerIndex = normalizedPayers.findIndex((name) => name === normalizedPayer)
      if (payerIndex < 0 || payers.length === 0) {
        continue
      }
      ...
      if (payerShareCents <= 0) {
        continue
      }

      payerItems.push({
        menu_item_id: row.menu_item_id ?? null,
        name: splitMembers.length >= 2 ? `${itemName} (split)` : itemName,
        price: centsToMoney(payerShareCents),
        quantity: 1,
        is_vegetarian: veg,
      })
    }
    ...
    const requestedLooksLikeFullBill =
      approxEquals(requestedTarget, fullSubtotal) ||
      (requestedSubtotal > 0 && approxEquals(requestedSubtotal, fullSubtotal))
    ...
    if (requestedLooksLikeFullBill) {
      ...
      if ((existingOrder ?? []).length > 0) {
        return json({ error: 'This group order has already been paid.' }, 409)
      }
      ...
      if (!customerName) {
        return json({ error: 'customer_name is required for split payments.' }, 400)
      }
      if (payerSubtotal <= 0) {
        return json({ error: 'No payable items found for this guest.' }, 400)
      }
      ...
      if ((existingGuestOrder ?? []).length > 0) {
        return json({ error: 'This guest has already paid their share.' }, 409)
      }

      // Allow paying extra (for example, covering remaining shares) but never
      // less than this payer's server-calculated share.
      if (
        requestedSubtotal > payerSubtotal + EPSILON &&
        requestedSubtotal <= fullSubtotal + EPSILON
      ) {
    ...
    if (!authedUserId) {
      return json({ error: 'Authentication is required to create checkout.' }, 401)
    }
    ...
    const normalizedItems = incomingCart.map((item) => ({
    ...
    const menuById = new Map(
      (menuRows ?? []).map((row) => [
        Number(row.id),
        {
          name: sanitizeLabel(row.name, 'Unknown Item', 120),
    ...
    if (missingIds.length > 0) {
      return json({ error: 'One or more menu items are invalid for this restaurant.' }, 400)
    }
      ...
      line_items: [
        {
          price_data: {
            currency: 'usd',
            product_data: {
              name: `Order at ${sanitizeLabel(restaurant.name, 'Rasvia Partner', 120)}`,
            },
