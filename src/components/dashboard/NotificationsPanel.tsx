...
function EmptyState({ tab }: { tab: Tab }) {
  return (
    ...
    >
      ...
      <p className="text-sm font-medium text-zinc-500">No {tab} notifications yet</p>
          ...
          {tabs.map(({ id, label, icon: Icon, count }) => {
            ...
            return (
              <motion.button
                ...
                className={`relative flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold transition-colors ${isActive ? "text-zinc-100" : "text-zinc-500 hover:text-zinc-300"
                  }`}
              >
                {isActive && (
                  <motion.div
